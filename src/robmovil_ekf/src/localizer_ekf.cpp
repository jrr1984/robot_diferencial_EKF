...
void robmovil_ekf::LocalizerEKF::makeA(void)
{
  ...
  A(1,3) = -sin(x(3)) * u(1) * delta_t;
  A(2,3) = cos(x(3)) * u(1) * delta_t;
...
{  
  ...
  if (diff_robot_landmark.length2() < 0.001)
  {
    ...
    H(2,1) = diff_robot_landmark.getY() / sqrt(diff_robot_landmark.getX()*diff_robot_landmark.getX() + diff_robot_landmark.getY()*diff_robot_landmark.getY());
    H(2,2) = -diff_robot_landmark.getX() / sqrt(diff_robot_landmark.getX()*diff_robot_landmark.getX() + diff_robot_landmark.getY()*diff_robot_landmark.getY());
  }
...
void robmovil_ekf::LocalizerEKF::makeProcess(void)
{
  ...
  x(1) = x(1) + u(1) * delta_t * cos(x(3));
  x(2) = x(2) + u(1) * delta_t * sin(x(3));
  x(3) = angles::normalize_angle(x(3) + u(2) * delta_t);
...
tf::Point robmovil_ekf::LocalizerEKF::measure2landmark(const LocalizerEKF::Vector& measure)
{
  ...
  double landmark_rot_x = cos(robot_theta) * landmark_x_robot - sin(robot_theta) * landmark_y_robot;
  double landmark_rot_y = sin(robot_theta) * landmark_x_robot + cos(robot_theta) * landmark_y_robot;
  ...
  predicted_landmark.setX(robot_x + landmark_rot_x);
  predicted_landmark.setY(robot_y + landmark_rot_y);
